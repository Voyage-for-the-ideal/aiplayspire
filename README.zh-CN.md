...
> **Ascension 10 评估（n = 20 局随机种子）：** AIPlaySpire 在 **90%** 的对局中**到达最终 Boss**，平均层数推进约为 **LLM-only baseline 的 2 倍**（*同一批种子*）。
>
> _"到达最终 Boss"不是胜率 —— 详见[评估](#评估)。_

---

## 评估

| 方法 | 最终 Boss 到达率 (A10) | 平均层数推进（同种子） |
|---|---|---|
| **AIPlaySpire** | **90%** | **~2.0×**（相对 LLM-only baseline） |
| LLM-only baseline | — | 1.0× |

评估协议：

- **难度：** Ascension 10（A10）。**20 局**随机抽取种子。
- **配对比较：** LLM-only baseline 使用完全相同的 20 个种子，因此每一对共享相同的游戏 RNG。
- **"到达最终 Boss"** 定义为：到达 Act 3 Boss 房间，或（走心路线时）到达腐化之心 (Corrupt Heart)。它衡量的是生存/推进程度 —— *不是*通关率。
- **n = 20** 样本量较小，请将结果视为方向性证据。Baseline 的到达率尚未记录，故表中为 "—"。

由于两个 agent 使用完全相同的种子，层数推进的差距是配对测量结果，而非种子运气。本项目的核心论点：长时程决策需要学得的价值函数，而不是文本生成；上述配对协议正是为了检验这一论点而设计，随着评估局数增加样本量会继续增长。
