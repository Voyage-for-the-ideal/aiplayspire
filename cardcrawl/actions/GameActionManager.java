...
public class GameActionManager {
    ...
    public static int totalDiscardedThisTurn = 0, damageReceivedThisTurn = 0, damageReceivedThisCombat = 0;
    ...
    public void removeFromQueue(AbstractCard c) {
        ...
        for (int i = 0; i < this.cardQueue.size(); i++) {
            if (((CardQueueItem) this.cardQueue.get(i)).card != null
                    && ((CardQueueItem) this.cardQueue.get(i)).card.equals(c)) {
                index = i;
    ...
    public void clearPostCombatActions() {
        for (Iterator<AbstractGameAction> i = this.actions.iterator(); i.hasNext();) {
            ...
            if (e instanceof com.megacrit.cardcrawl.actions.common.HealAction
                    || e instanceof com.megacrit.cardcrawl.actions.common.GainBlockAction || e instanceof UseCardAction
    ...
    private void getNextAction() {
        if (!this.actions.isEmpty()) {
            ...
            if (c != null && ((CardQueueItem) this.cardQueue.get(0)).randomTarget) {
                ((CardQueueItem) this.cardQueue.get(0)).monster = AbstractDungeon.getMonsters().getRandomMonster(null,
                        true, AbstractDungeon.cardRandomRng);
            }

            if (((CardQueueItem) this.cardQueue.get(0)).card != null
                    && (c.canUse(AbstractDungeon.player, ((CardQueueItem) this.cardQueue.get(0)).monster) ||
                ...
                ((CardQueueItem) this.cardQueue.get(0)).card.energyOnUse = ((CardQueueItem) this.cardQueue
                        .get(0)).energyOnUse;
                if (c.isInAutoplay) {

                    ((CardQueueItem) this.cardQueue.get(0)).card.ignoreEnergyOnUse = true;
                } else {
                    ((CardQueueItem) this.cardQueue.get(0)).card.ignoreEnergyOnUse = ((CardQueueItem) this.cardQueue
                ...
                if (!((CardQueueItem) this.cardQueue.get(0)).card.dontTriggerOnUseCard) {
                    ...
                    for (AbstractBlight b : AbstractDungeon.player.blights) {
                        b.onPlayCard(((CardQueueItem) this.cardQueue.get(0)).card,
                                ((CardQueueItem) this.cardQueue.get(0)).monster);
                    }
                ...
                if (((CardQueueItem) this.cardQueue
                        .get(0)).card instanceof com.megacrit.cardcrawl.cards.tempCards.Shiv) {
                    int shivCount = 0;
                ...
                if (((CardQueueItem) this.cardQueue.get(0)).card != null) {
                    if (((CardQueueItem) this.cardQueue.get(0)).card.target == AbstractCard.CardTarget.ENEMY
                        ...
                        for (Iterator<AbstractCard> i = AbstractDungeon.player.limbo.group.iterator(); i.hasNext();) {
                            ...
                            if (e == ((CardQueueItem) this.cardQueue.get(0)).card) {
                                ...
                                AbstractDungeon.effectList
                                        .add(new ExhaustCardEffect(((CardQueueItem) this.cardQueue.get(0)).card));
                                i.remove();
                            }
                        }

                        if (((CardQueueItem) this.cardQueue.get(0)).monster == null) {
                            ((CardQueueItem) this.cardQueue.get(0)).card.drawScale = ((CardQueueItem) this.cardQueue
                                    .get(0)).card.targetDrawScale;
                            ((CardQueueItem) this.cardQueue.get(0)).card.angle = ((CardQueueItem) this.cardQueue
                                    .get(0)).card.targetAngle;
                            ((CardQueueItem) this.cardQueue.get(0)).card.current_x = ((CardQueueItem) this.cardQueue
                                    .get(0)).card.target_x;
                            ((CardQueueItem) this.cardQueue.get(0)).card.current_y = ((CardQueueItem) this.cardQueue
                                    .get(0)).card.target_y;
                            AbstractDungeon.effectList
                                    .add(new ExhaustCardEffect(((CardQueueItem) this.cardQueue.get(0)).card));
                        }
                        ...
                        AbstractDungeon.player.useCard(((CardQueueItem) this.cardQueue
                                .get(0)).card,
                                ((CardQueueItem) this.cardQueue
                                        .get(0)).monster,
                                ((CardQueueItem) this.cardQueue
                ...
                if (c != null) {
                    AbstractDungeon.effectList.add(new ThoughtBubble(AbstractDungeon.player.dialogX,
                            AbstractDungeon.player.dialogY, 3.0F, c.cantUseMessage, true));
                }
            }
            ...
            if (!canPlayCard && c != null && c.isInAutoplay) {
                ...
                AbstractDungeon.actionManager.addToBottom((AbstractGameAction) new UseCardAction(c));
            }
            ...
            if (!m.isDeadOrEscaped() || m.halfDead) {
                if (m.intent != AbstractMonster.Intent.NONE) {
                    addToBottom((AbstractGameAction) new ShowMoveNameAction(m));
                    addToBottom(new IntentFlashAction(m));
                }

                if (!((Boolean) TipTracker.tips.get("INTENT_TIP")).booleanValue()
            ...
            if (this.monsterQueue.isEmpty()) {
                addToBottom((AbstractGameAction) new WaitAction(1.5F));
            ...
            if (!(AbstractDungeon.getCurrRoom()).isBattleOver) {
                addToBottom((AbstractGameAction) new DrawCardAction(null, AbstractDungeon.player.gameHandSize, true));
                AbstractDungeon.player.applyStartOfTurnPostDrawRelics();
                AbstractDungeon.player.applyStartOfTurnPostDrawPowers();
                addToBottom((AbstractGameAction) new EnableEndTurnButtonAction());
            }
        }
    }
    ...
    private void callEndOfTurnActions() {
        ...
        addToBottom((AbstractGameAction) new TriggerEndOfTurnOrbsAction());
    ...
    public void callEndTurnEarlySequence() {
        for (CardQueueItem i : AbstractDungeon.actionManager.cardQueue) {
            ...
            if (i.autoplayCard) {
                ...
                AbstractDungeon.actionManager.addToBottom((AbstractGameAction) new UseCardAction(i.card));
            }
        }
    ...
    public void cleanCardQueue() {
        for (Iterator<CardQueueItem> iterator = this.cardQueue.iterator(); iterator.hasNext();) {
            CardQueueItem e = iterator.next();
    ...
    public static void queueExtraCard(AbstractCard card, AbstractMonster m) {
        ...
        switch (extraCount) {
            ...
            case 1:
                ...
                break;
            case 2:
                tmp.target_x = Settings.WIDTH / 2.0F - 600.0F * Settings.xScale;
                break;
            case 3:
                tmp.target_x = Settings.WIDTH / 2.0F + 600.0F * Settings.xScale;
                break;
            ...
            default:
                tmp.target_x = MathUtils.random(Settings.WIDTH * 0.2F, Settings.WIDTH * 0.8F);
                tmp.target_y = MathUtils.random(Settings.HEIGHT * 0.3F, Settings.HEIGHT * 0.7F);
                break;
        }
 ...
 * GameActionManager.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
