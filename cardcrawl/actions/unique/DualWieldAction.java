...
public class DualWieldAction
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_FAST) {
            ...
            if (this.p.hand.group.size() == 1) {
                for (int i = 0; i < this.dupeAmount; i++) {
                    addToTop((AbstractGameAction) new MakeTempCardInHandAction(
                            this.p.hand.getTopCard().makeStatEquivalentCopy()));
                }
                returnCards();
                this.isDone = true;
            }
 ...
 * DualWieldAction.class Java compiler version: 8 (52.0) JD-Core Version: 1.1.3
 */
