...
public class HemokinesisParticle
    ...
    public HemokinesisParticle(float sX, float sY, float tX, float tY, boolean facingLeft) {
        ...
        this.crs.controlPoints = (Vector2[]) new Vector2[1];
    ...
    private void updateMovement() {
        ...
        if (!this.stopRotating) {
            ...
            if (!this.stopRotating &&
                    Math.abs(this.rotation - targetAngle) < Gdx.graphics.getDeltaTime() * this.rotationRate) {
            ...
            for (int i = 0; i < 5; i++) {
                if (this.facingLeft) {
                    AbstractDungeon.effectsQueue
                            .add(new DamageImpactLineEffect(this.target.x + DST_THRESHOLD, this.target.y));
                } else {
                    AbstractDungeon.effectsQueue
                            .add(new DamageImpactLineEffect(this.target.x - DST_THRESHOLD, this.target.y));
                }
        ...
        if (this.controlPoints.size() > 3) {
            ...
            this.crs.set((Vector2[]) this.controlPoints.toArray((Object[]) vec2Array), false);
            for (int i = 0; i < 60; i++) {
                this.points[i] = new Vector2();
                this.crs.valueAt((Vector2) this.points[i], i / 59.0F);
            }
        }
        ...
        if (!this.isDone) {
            ...
            for (i = this.points.length - 1; i > 0; i--) {
                if (this.points[i] != null) {
                    sb.draw((TextureRegion) this.img, (this.points[i]).x - (this.img.packedWidth / 2),
                            (this.points[i]).y - (this.img.packedHeight / 2), this.img.packedWidth / 2.0F,
                            this.img.packedHeight / 2.0F, this.img.packedWidth, this.img.packedHeight, scaleCpy * 1.5F,
                            scaleCpy * 1.5F, this.rotation);
            ...
            for (i = this.points.length - 1; i > 0; i--) {
                if (this.points[i] != null) {
                    sb.draw((TextureRegion) this.img, (this.points[i]).x - (this.img.packedWidth / 2),
                            (this.points[i]).y - (this.img.packedHeight / 2), this.img.packedWidth / 2.0F,
                            this.img.packedHeight / 2.0F, this.img.packedWidth, this.img.packedHeight, scaleCpy,
                            scaleCpy, this.rotation);
 ...
 * HemokinesisParticle.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
