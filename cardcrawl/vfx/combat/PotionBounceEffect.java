...
public class PotionBounceEffect extends AbstractGameEffect {
    ...
    public PotionBounceEffect(float srcX, float srcY, float destX, float destY) {
        ...
        this.color = new Color(0.4F, 1.0F, 0.0F, 0.0F);
    ...
    public void update() {
        ...
        if (this.duration > 0.3F) {
            ...
            this.yOffset = Interpolation.circleIn.apply(this.bounceHeight, 0.0F, (this.duration - 0.3F) / 0.3F)
                    * Settings.scale;
        } else {
            this.yOffset = Interpolation.circleOut.apply(0.0F, this.bounceHeight, this.duration / 0.3F)
                    * Settings.scale;
        }
    ...
    public void render(SpriteBatch sb) {
        ...
        for (int i = 5; i < this.previousPos.size(); i++) {
            sb.draw((TextureRegion) ImageMaster.POWER_UP_2, ((Vector2) this.previousPos
                    ...
                    .get(i)).x - (img.packedWidth / 2),
                    ((Vector2) this.previousPos
                            .get(i)).y - (img.packedHeight / 2),
                    img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight,
                    this.scale / 40.0F / i, this.scale / 40.0F / i, this.rotation);
        }

        sb.setColor(this.color);
        sb.draw((TextureRegion) img, this.cX - (img.packedWidth / 2), this.cY - (img.packedHeight / 2) + this.yOffset,
                img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth, img.packedHeight, this.scale,
 ...
 * PotionBounceEffect.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
