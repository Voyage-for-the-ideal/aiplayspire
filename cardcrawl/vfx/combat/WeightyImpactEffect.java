...
public class WeightyImpactEffect
    ...
    public void update() {
        ...
        if (this.duration < 0.0F) {
            ...
            if (!this.impactHook) {
                ...
                int i;
                for (i = 0; i < 5; i++) {
                    AbstractDungeon.effectsQueue.add(new DamageImpactCurvyEffect(this.x + img.packedWidth / 2.0F,
                            this.y + img.packedWidth / 2.0F));
                }

                for (i = 0; i < 30; i++) {
                    AbstractDungeon.effectsQueue.add(new UpgradeShineParticleEffect(this.x +

                            MathUtils.random(-100.0F, 100.0F) * Settings.scale + img.packedWidth / 2.0F,
                            this.y +
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw((TextureRegion) img, this.x, this.y + 140.0F * Settings.scale, img.packedWidth / 2.0F,
                img.packedHeight / 2.0F, img.packedWidth, img.packedHeight * (this.duration + 0.2F) * 5.0F, this.scale *

                        MathUtils.random(0.99F, 1.01F) * 0.3F,
                this.scale *
                        MathUtils.random(0.99F, 1.01F) * 2.0F * (this.duration + 0.8F),
                this.rotation);

        this.color.g = 0.6F;
        sb.setColor(this.color);
        sb.draw((TextureRegion) img, this.x, this.y + 40.0F * Settings.scale, img.packedWidth / 2.0F,
                img.packedHeight / 2.0F, img.packedWidth, img.packedHeight * (this.duration + 0.2F) * 5.0F, this.scale *

                        MathUtils.random(0.99F, 1.01F) * 0.7F,
                this.scale *
                        MathUtils.random(0.99F, 1.01F) * 1.3F * (this.duration + 0.8F),
                this.rotation);

        this.color.g = 0.2F;
        sb.setColor(this.color);
        sb.draw((TextureRegion) img, this.x, this.y, img.packedWidth / 2.0F, img.packedHeight / 2.0F, img.packedWidth,
                img.packedHeight * (this.duration + 0.2F) * 5.0F, this.scale *

                        MathUtils.random(0.99F, 1.01F),
                this.scale *
                        MathUtils.random(0.99F, 1.01F) * (this.duration + 0.8F),
                this.rotation);
 ...
 * WeightyImpactEffect.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
