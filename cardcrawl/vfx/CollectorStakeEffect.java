...
public class CollectorStakeEffect extends AbstractGameEffect {
    ...
    public CollectorStakeEffect(float x, float y) {
        ...
        this.color = new Color(MathUtils.random(0.5F, 1.0F), MathUtils.random(0.0F, 0.4F), MathUtils.random(0.5F, 1.0F),
                0.0F);
    }
    private float tX;
    private float tY;
    ...
    public void update() {
        ...
        if (this.duration > 0.5F) {
            this.scale = Interpolation.elasticIn.apply(this.targetScale, this.targetScale * 10.0F,
                    (this.duration - 0.5F) * 2.0F) * Settings.scale;
        ...
        if (this.duration < 0.05F &&
                !this.shownSlash) {
            AbstractDungeon.effectsQueue.add(new AdditiveSlashImpactEffect(this.tX + img.packedWidth / 2.0F,
                    this.tY + img.packedHeight / 2.0F, this.color
                            .cpy()));
 ...
 * CollectorStakeEffect.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
