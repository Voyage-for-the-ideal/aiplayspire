...
public class FireFlyEffect extends AbstractGameEffect {
    ...
    public void update() {
        ...
        if (!this.prevPositions.isEmpty() && (((Vector2) this.prevPositions.get(0)).x < 0.0F
                || ((Vector2) this.prevPositions.get(0)).x > Settings.WIDTH)) {
            this.isDone = true;
        }

        this.y += this.vY * Gdx.graphics.getDeltaTime();
        this.y += MathUtils.sin(this.duration * this.waveDst) * this.waveDst / 4.0F * Gdx.graphics.getDeltaTime()
                * 60.0F;
    ...
    public void render(SpriteBatch sb) {
        ...
        for (int i = this.prevPositions.size() - 1; i > 0; i--) {
            ...
            sb.draw((TextureRegion) this.img, ((Vector2) this.prevPositions
                    ...
                    this.img.packedHeight, this.scale * (i + 5) / this.prevPositions

                            .size(),
                    this.scale * (i + 5) / this.prevPositions
                            .size(),
                    this.rotation);
        }
 ...
 * FireFlyEffect.class Java compiler version: 8 (52.0) JD-Core Version: 1.1.3
 */
