...
public class CampfireUI
    ...
    private void updateControllerInput() {
        if (!Settings.isControllerMode || AbstractDungeon.player.viewingRelics
                || AbstractDungeon.topPanel.selectPotionMode || !AbstractDungeon.topPanel.potionUi.isHidden
                || this.somethingSelected || this.buttons
    ...
    private void updateCharacterPosition() {
        ...
        AbstractDungeon.player.animX = Interpolation.exp10In.apply(0.0F, -300.0F * Settings.scale,
                this.charAnimTimer / 2.0F);
    }
    ...
    public void render(SpriteBatch sb) {
        if (!hidden) {
            ...
            for (CampfireBubbleEffect e : this.bubbles) {
                e.render(sb, 950.0F * Settings.xScale,
                        Settings.HEIGHT / 2.0F + 60.0F * Settings.yScale + this.effect.y / 4.0F);
            }

            FontHelper.renderFontCentered(sb, FontHelper.losePowerFont, this.bubbleMsg, 950.0F * Settings.xScale,
                    Settings.HEIGHT / 2.0F + 310.0F * Settings.scale + this.effect.y / 3.0F, Settings.CREAM_COLOR,
                    1.2F);
    ...
    private void renderCampfireButtons(SpriteBatch sb) {
        ...
        int indexToStartAt = Math.max(
                Math.min(
                        (int) ((maxPossibleStartingIndex + 1) * MathHelper
                                .percentFromValueBetween(this.scrollLowerBound, this.scrollUpperBound, this.scrollY)),
        ...
        for (AbstractCampfireOption co : this.buttons) {
            if (this.buttons.indexOf(co) >= indexToStartAt && this.buttons.indexOf(co) < indexToStartAt + 6) {
                ...
                if (this.buttons.indexOf(co) == this.buttons.size() - 1
                        && (this.buttons.size() - indexToStartAt) % 2 == 1) {
