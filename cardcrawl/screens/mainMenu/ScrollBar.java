...
public class ScrollBar {
    ...
    public ScrollBar(ScrollBarListener listener) {
        this(listener, Settings.WIDTH - 150.0F * Settings.scale - TRACK_W / 2.0F, Settings.HEIGHT / 2.0F,
                Settings.HEIGHT - 256.0F * Settings.scale);

        this.cursorDrawPosition = getYPositionForPercent(0.0F);
    }
    ...
    public void positionWithinOnRight(float right, float top, float bottom) {
        ...
        changeHeight(top - bottom - 2.0F * this.TRACK_CAP_VISIBLE_HEIGHT);
        reset();
    }
    ...
    public void reset() {
        this.cursorDrawPosition = getYPositionForPercent(this.currentScrollPercent);
    }
    ...
    private void renderCursor(SpriteBatch sb) {
        ...
        float yForPercent = getYPositionForPercent(this.currentScrollPercent);
        ...
        if (this.isInteractable) {
            ...
            sb.draw(ImageMaster.SCROLL_BAR_TRAIN, x + this.CURSOR_W / 4.0F, this.cursorDrawPosition,
                    this.CURSOR_W / 2.0F, this.CURSOR_H);
            ShaderHelper.setShader(sb, ShaderHelper.Shader.DEFAULT);
        }
    }

    private void renderTrack(SpriteBatch sb) {
        sb.draw(ImageMaster.SCROLL_BAR_MIDDLE, this.sliderHb.x, this.sliderHb.y, this.sliderHb.width,
                this.sliderHb.height);
        sb.draw(ImageMaster.SCROLL_BAR_TOP, this.sliderHb.x, this.sliderHb.y + this.sliderHb.height,
                this.sliderHb.width, this.TRACK_CAP_HEIGHT);
        sb.draw(ImageMaster.SCROLL_BAR_BOTTOM, this.sliderHb.x, this.sliderHb.y - this.TRACK_CAP_HEIGHT,
                this.sliderHb.width, this.TRACK_CAP_HEIGHT);
    }
 ...
 * ScrollBar.class Java compiler version: 8 (52.0) JD-Core Version: 1.1.3
 */
