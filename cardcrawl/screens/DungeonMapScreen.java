...
public class DungeonMapScreen {
    ...
    public void update() {
        if (this.scrollWaitTimer < 0.0f && Settings.isControllerMode
                && AbstractDungeon.screen == AbstractDungeon.CurrentScreen.MAP && !this.map.legend.isLegendHighlighted
        ...
        if (AbstractDungeon.screen == AbstractDungeon.CurrentScreen.MAP && !this.dismissable
    ...
    private void updateMouse() {
        ...
        if (InputHelper.justReleasedClickLeft && this.clickTimer < 0.4f && Vector2.dst(this.clickStartX,
                this.clickStartY, InputHelper.mX, InputHelper.mY) < Settings.CLICK_DIST_THRESHOLD) {
            this.clicked = true;
        }
        if (InputHelper.justClickedLeft || (CInputActionSet.select.isJustPressed()
                && AbstractDungeon.topPanel.potionUi.isHidden && !AbstractDungeon.topPanel.selectPotionMode)) {
        ...
        if (CInputActionSet.select.isJustPressed() && this.clickTimer < 0.4f
                && AbstractDungeon.screen == AbstractDungeon.CurrentScreen.MAP) {
            this.clicked = true;
        }
    }
    ...
    private void updateControllerInput() {
        if (this.scrollWaitTimer > 0.0f || !Settings.isControllerMode || AbstractDungeon.topPanel.selectPotionMode
                || !AbstractDungeon.topPanel.potionUi.isHidden || this.map.legend.isLegendHighlighted
            ...
            if (CInputActionSet.left.isJustPressed() || CInputActionSet.right.isJustPressed()
                    || CInputActionSet.altRight.isJustPressed() || CInputActionSet.altLeft.isJustPressed()) {
            ...
            if (AbstractDungeon.screen == AbstractDungeon.CurrentScreen.MAP) {
                ...
                if (!AbstractDungeon.firstRoomChosen) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        boolean flightMatters = AbstractDungeon.player.hasRelic(WingBoots.ID)
                                || ModHelper.isModEnabled("Flight");
                        if (AbstractDungeon.currMapNode.isConnectedTo(n2)
                                || (flightMatters && AbstractDungeon.currMapNode.wingedIsConnectedTo(n2))) {
                            nodes.add(n2);
                        }
                    }
                }
                ...
                if (!anyHovered && this.mapNodeHb == null && !nodes.isEmpty()) {
                    Gdx.input.setCursorPosition((int) nodes.get(nodes.size() / 2).hb.cX,
                            Settings.HEIGHT - ((int) nodes.get(nodes.size() / 2).hb.cY));
                    this.mapNodeHb = nodes.get(nodes.size() / 2).hb;
                } else if (!anyHovered && nodes.isEmpty()) {
                    Gdx.input.setCursorPosition((int) AbstractDungeon.dungeonMapScreen.map.bossHb.cX,
                            Settings.HEIGHT - ((int) AbstractDungeon.dungeonMapScreen.map.bossHb.cY));
                    this.mapNodeHb = null;
                    ...
                    Gdx.input.setCursorPosition((int) nodes.get(index2).hb.cX,
                            Settings.HEIGHT - ((int) nodes.get(index2).hb.cY));
                    ...
                    Gdx.input.setCursorPosition((int) nodes.get(index3).hb.cX,
                            Settings.HEIGHT - ((int) nodes.get(index3).hb.cY));
    ...
    private void updateAnimation() {
        ...
        if (this.scrollWaitTimer < 0.0f) {
            ...
            offsetY = Interpolation.exp10.apply(MAP_SCROLL_LOWER, this.mapScrollUpperLimit,
                    this.scrollWaitTimer / 3.0f);
        }
    }
    ...
    private void oscillateColor() {
        ...
        this.oscillatingColor.a = (0.33f + ((MathUtils.cos(this.oscillatingTimer) + 1.0f) / 3.0f))
                * this.oscillatingFader;
    }
    ...
    public void renderReticle(SpriteBatch sb, Hitbox hb) {
        renderReticleCorner(sb, ((-hb.width) / 2.0f) - RETICLE_DIST, (hb.height / 2.0f) + RETICLE_DIST, hb, false,
                false);
