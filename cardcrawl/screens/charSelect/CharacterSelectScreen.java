...
public class CharacterSelectScreen {
    ...
    public static final String[] A_TEXT = uiStrings2.TEXT;
    ...
    public void initialize() {
        this.options.add(new CharacterOption(TEXT[2], CardCrawlGame.characterManager

                .recreateCharacter(AbstractPlayer.PlayerClass.IRONCLAD), ImageMaster.CHAR_SELECT_IRONCLAD,
                ImageMaster.CHAR_SELECT_BG_IRONCLAD));

        if (!UnlockTracker.isCharacterLocked("The Silent")) {
            this.options.add(new CharacterOption(TEXT[3], CardCrawlGame.characterManager

                    .recreateCharacter(AbstractPlayer.PlayerClass.THE_SILENT), ImageMaster.CHAR_SELECT_SILENT,
        ...
        if (!UnlockTracker.isCharacterLocked("Defect")) {
            this.options.add(new CharacterOption(TEXT[4], CardCrawlGame.characterManager

                    .recreateCharacter(AbstractPlayer.PlayerClass.DEFECT), ImageMaster.CHAR_SELECT_DEFECT,
        ...
        if (!Settings.isMobile) {
            this.ascensionModeHb.move(Settings.WIDTH / 2.0F - ASC_LEFT_W / 2.0F - 50.0F * Settings.scale,
    ...
    private void positionButtons() {
        ...
        for (int i = 0; i < count; i++) {
            if (Settings.isMobile) {
                ((CharacterOption) this.options.get(i)).hb.move(offsetX + i * 220.0F * Settings.scale,
                        230.0F * Settings.yScale);
            } else {
                ((CharacterOption) this.options.get(i)).hb.move(offsetX + i * 220.0F * Settings.scale,
    ...
    public void update() {
        if (this.ascLeftHb != null) {
            if (!Settings.isMobile) {
                this.ascLeftHb.move(Settings.WIDTH / 2.0F + 200.0F * Settings.scale - ASC_RIGHT_W * 0.25F,
                        70.0F * Settings.scale);
                this.ascRightHb.move(Settings.WIDTH / 2.0F + 200.0F * Settings.scale + ASC_RIGHT_W * 1.25F,
                        70.0F * Settings.scale);
            } else {
    ...
    public void render(SpriteBatch sb) {
        ...
        if (this.bgCharImg != null) {
            if (Settings.isSixteenByTen) {
                sb.draw(this.bgCharImg, Settings.WIDTH / 2.0F - 960.0F, Settings.HEIGHT / 2.0F - 600.0F, 960.0F, 600.0F,
                        1920.0F, 1200.0F, Settings.scale, Settings.scale, 0.0F, 0, 0, 1920, 1200, false, false);

            } else if (Settings.isFourByThree) {
                sb.draw(this.bgCharImg, Settings.WIDTH / 2.0F - 960.0F,
                        Settings.HEIGHT / 2.0F - 600.0F + this.bg_y_offset, 960.0F, 600.0F, 1920.0F, 1200.0F,
                        Settings.yScale, Settings.yScale, 0.0F, 0, 0, 1920, 1200, false, false);

            } else if (Settings.isLetterbox) {
                sb.draw(this.bgCharImg, Settings.WIDTH / 2.0F - 960.0F,
                        Settings.HEIGHT / 2.0F - 600.0F + this.bg_y_offset, 960.0F, 600.0F, 1920.0F, 1200.0F,
                        Settings.xScale, Settings.xScale, 0.0F, 0, 0, 1920, 1200, false, false);

            } else {

                sb.draw(this.bgCharImg, Settings.WIDTH / 2.0F - 960.0F,
                        Settings.HEIGHT / 2.0F - 600.0F + this.bg_y_offset, 960.0F, 600.0F, 1920.0F, 1200.0F,
                        Settings.scale, Settings.scale, 0.0F, 0, 0, 1920, 1200, false, false);
            }
        }
        ...
        if (!this.seedPanel.shown &&
                !anythingSelected) {
            if (!Settings.isMobile) {
                FontHelper.renderFontCentered(sb, FontHelper.losePowerFont, CHOOSE_CHAR_MSG, Settings.WIDTH / 2.0F,
                        340.0F * Settings.yScale, Settings.CREAM_COLOR, 1.2F);

            } else {

                FontHelper.renderFontCentered(sb, FontHelper.losePowerFont, CHOOSE_CHAR_MSG, Settings.WIDTH / 2.0F,
                        380.0F * Settings.yScale, Settings.CREAM_COLOR, 1.2F);
            }
        }
    }
    ...
    private void renderSeedSettings(SpriteBatch sb) {
        ...
        if (this.seedHb.hovered) {
            ...
            TipHelper.renderGenericTip(InputHelper.mX + 50.0F * Settings.scale,
                    InputHelper.mY + 100.0F * Settings.scale, TEXT[11], TEXT[12]);
        }

        if (!Settings.isControllerMode) {
            if (Settings.seedSet) {
                FontHelper.renderSmartText(sb, FontHelper.cardTitleFont, TEXT[10], SEED_X, SEED_Y, 9999.0F, 0.0F,
                        textColor);
                ...
                FontHelper.renderFontLeftTopAligned(sb, FontHelper.cardTitleFont,
                        ...
                        SeedHelper.getUserFacingSeedString(), SEED_X - 30.0F * Settings.scale +
                                FontHelper.getSmartWidth(FontHelper.cardTitleFont, TEXT[13], 9999.0F, 0.0F),
                        90.0F * Settings.scale, Settings.BLUE_TEXT_COLOR);

            } else {

                FontHelper.renderSmartText(sb, FontHelper.cardTitleFont, TEXT[13], SEED_X, SEED_Y, 9999.0F, 0.0F,
                        textColor);
            ...
            if (Settings.seedSet) {
                FontHelper.renderSmartText(sb, FontHelper.cardTitleFont, TEXT[10], SEED_X + 100.0F * Settings.scale,
                        SEED_Y, 9999.0F, 0.0F, textColor);
                ...
                FontHelper.renderSmartText(sb, FontHelper.cardTitleFont, TEXT[13], SEED_X + 100.0F * Settings.scale,
                        SEED_Y, 9999.0F, 0.0F, textColor);
            }

            sb.draw(ImageMaster.CONTROLLER_LT, 80.0F * Settings.scale - 32.0F, 80.0F * Settings.scale - 32.0F, 32.0F,
    ...
    private void renderAscensionMode(SpriteBatch sb) {
        ...
        if (this.isAscensionModeUnlocked) {
            if (!Settings.isMobile) {
                sb.draw(ImageMaster.OPTION_TOGGLE, Settings.WIDTH / 2.0F - ASC_LEFT_W - 16.0F - 30.0F * Settings.scale,
                        this.ascensionModeHb.cY - 16.0F, 16.0F, 16.0F, 32.0F, 32.0F, Settings.scale, Settings.scale,
                        0.0F, 0, 0, 32, 32, false, false);

            } else {

                sb.draw(ImageMaster.CHECKBOX, Settings.WIDTH / 2.0F - ASC_LEFT_W - 36.0F * Settings.scale - 32.0F,
                        this.ascensionModeHb.cY - 32.0F, 32.0F, 32.0F, 64.0F, 64.0F, Settings.scale * 0.9F,
                        Settings.scale * 0.9F, 0.0F, 0, 0, 64, 64, false, false);
            }

            if (this.ascensionModeHb.hovered) {
                FontHelper.renderFontCentered(sb, FontHelper.cardTitleFont, TEXT[6],
                        Settings.WIDTH / 2.0F - ASC_LEFT_W / 2.0F, this.ascensionModeHb.cY, Settings.GREEN_TEXT_COLOR);

                TipHelper.renderGenericTip(InputHelper.mX - 140.0F * Settings.scale,
                        InputHelper.mY + 340.0F * Settings.scale, TEXT[8], TEXT[9]);

            } else {

                FontHelper.renderFontCentered(sb, FontHelper.cardTitleFont, TEXT[6],
                        Settings.WIDTH / 2.0F - ASC_LEFT_W / 2.0F, this.ascensionModeHb.cY, Settings.GOLD_COLOR);
            }

            FontHelper.renderFontCentered(sb, FontHelper.cardTitleFont, TEXT[7] + this.ascensionLevel,
                    Settings.WIDTH / 2.0F + ASC_RIGHT_W / 2.0F + 200.0F * Settings.scale, this.ascensionModeHb.cY,
                    Settings.BLUE_TEXT_COLOR);

            if (this.isAscensionMode) {
                sb.setColor(Color.WHITE);
                if (!Settings.isMobile) {
                    sb.draw(ImageMaster.OPTION_TOGGLE_ON,
                    ...
                    sb.draw(ImageMaster.TICK, Settings.WIDTH / 2.0F - ASC_LEFT_W - 36.0F * Settings.scale - 32.0F,
                            this.ascensionModeHb.cY - 32.0F, 32.0F, 32.0F, 64.0F, 64.0F, Settings.scale * 0.9F,
                            Settings.scale * 0.9F, 0.0F, 0, 0, 64, 64, false, false);
                }

                if (Settings.isMobile) {
                    FontHelper.renderFontCentered(sb, FontHelper.smallDialogOptionFont, this.ascLevelInfoString,
                            Settings.WIDTH / 2.0F, 60.0F * Settings.scale, Settings.CREAM_COLOR);

                } else {

                    FontHelper.renderFontCentered(sb, FontHelper.cardDescFont_N, this.ascLevelInfoString,
                            Settings.WIDTH / 2.0F, 35.0F * Settings.scale, Settings.CREAM_COLOR);
                }
            }
            ...
            sb.draw(ImageMaster.CF_LEFT_ARROW, this.ascLeftHb.cX - 24.0F, this.ascLeftHb.cY - 24.0F, 24.0F, 24.0F,
            ...
            sb.draw(ImageMaster.CF_RIGHT_ARROW, this.ascRightHb.cX - 24.0F, this.ascRightHb.cY - 24.0F, 24.0F, 24.0F,
                    48.0F, 48.0F, Settings.scale, Settings.scale, 0.0F, 0, 0, 48, 48, false, false);

            if (Settings.isControllerMode) {
                sb.draw(CInputActionSet.proceed
                        .getKeyImg(), this.ascensionModeHb.cX - 100.0F * Settings.scale - 32.0F,
                        this.ascensionModeHb.cY - 32.0F, 32.0F, 32.0F, 64.0F, 64.0F, Settings.scale, Settings.scale,
                        0.0F, 0, 0, 64, 64, false, false);

                sb.draw(CInputActionSet.pageLeftViewDeck
                        .getKeyImg(), this.ascLeftHb.cX - 60.0F * Settings.scale - 32.0F, this.ascLeftHb.cY - 32.0F,
                ...
                sb.draw(CInputActionSet.pageRightViewExhaust
                        .getKeyImg(), this.ascRightHb.cX + 60.0F * Settings.scale - 32.0F, this.ascRightHb.cY - 32.0F,
                        32.0F, 32.0F, 64.0F, 64.0F, Settings.scale, Settings.scale, 0.0F, 0, 0, 64, 64, false, false);
            }
 ...
 * \CharacterSelectScreen.class Java compiler version: 8 (52.0) JD-Core Version:
 * 1.1.3
 */
