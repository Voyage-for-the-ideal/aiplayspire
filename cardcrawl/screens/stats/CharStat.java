...
public class CharStat {
    ...
    private static final AchievementStrings achievementStrings = CardCrawlGame.languagePack
            .getAchievementString("CharStat");
    ...
    public CharStat(ArrayList<CharStat> allChars) {
        ...
        for (CharStat stat : allChars) {
            ...
            if (stat.furthestAscent > highestFloorTmp) {
                this.furthestAscent = stat.furthestAscent;
                highestFloorTmp = this.furthestAscent;
            }

            if (stat.highestDaily > highestDailyTmp) {
                this.highestDaily = stat.highestDaily;
                highestDailyTmp = this.highestDaily;
            }
        ...
        this.info = TEXT[0] + formatHMSM(this.playTime) + " NL ";
        this.info += TEXT[1] + this.numVictory + " NL ";
        this.info += TEXT[2] + this.numDeath + " NL ";
        this.info += TEXT[3] + this.totalFloorsClimbed + " NL ";
        this.info += TEXT[4] + this.bossKilled + " NL ";
        this.info += TEXT[5] + this.enemyKilled + " NL ";

        this.info2 = TEXT[7] + UnlockTracker.getCardsSeenString() + " NL ";

        int unlockedCardCount = UnlockTracker.unlockedRedCardCount + UnlockTracker.unlockedGreenCardCount
                + UnlockTracker.unlockedBlueCardCount + UnlockTracker.unlockedPurpleCardCount;

        int lockedCardCount = UnlockTracker.lockedRedCardCount + UnlockTracker.lockedGreenCardCount
                + UnlockTracker.lockedBlueCardCount + UnlockTracker.lockedPurpleCardCount;

        this.info2 += TEXT[8] + unlockedCardCount + "/" + lockedCardCount + " NL ";
        this.info2 += TEXT[9] + UnlockTracker.getRelicsSeenString() + " NL ";
        this.info2 += TEXT[10] + UnlockTracker.unlockedRelicCount + "/" + UnlockTracker.lockedRelicCount + " NL ";

        if (this.fastestTime != 999999999999L) {
            this.info2 += TEXT[13] + formatHMSM(this.fastestTime) + " NL ";
        }
    }
    ...
    public CharStat(AbstractPlayer c) {
        ...
        this.cardsUnlocked = calculateCardsUnlocked(c);
        this.cardsDiscovered = getSeenCardCount(c);
        this.cardsToDiscover = getCardCountForChar(c);
        ...
        this.fastestTime = this.pref.getLong("FAST_VICTORY", 0L);
        this.highestScore = this.pref.getInteger("HIGHEST_SCORE", 0);

        this.info = TEXT[0] + formatHMSM(this.playTime) + " NL ";
        this.info += TEXT[7] + this.cardsDiscovered + "/" + this.cardsToDiscover + " NL ";
        this.info += TEXT[8] + this.cardsUnlocked + "/" + UnlockTracker.lockedRedCardCount + " NL ";

        if (this.fastestTime != 0L) {
            this.info += TEXT[13] + formatHMSM(this.fastestTime) + " NL ";
        }
        this.info += TEXT[23] + this.highestScore + " NL ";
        if (this.bestWinStreak > 0) {
            this.info += TEXT[22] + this.bestWinStreak + " NL ";
        }

        this.info2 = TEXT[17] + this.numVictory + " NL ";
        this.info2 += TEXT[18] + this.numDeath + " NL ";
        this.info2 += TEXT[19] + this.totalFloorsClimbed + " NL ";
        this.info2 += TEXT[20] + this.bossKilled + " NL ";
        this.info2 += TEXT[21] + this.enemyKilled + " NL ";
    ...
    public static String formatHMSM(float t) {
        ...
        if (hours > 0) {
            res = String.format(TEXT[24],
                    new Object[] {Integer.valueOf(hours), Integer.valueOf(minutes), Integer.valueOf(seconds) });
        } else {
            res = String.format(TEXT[25], new Object[] {Integer.valueOf(minutes), Integer.valueOf(seconds) });
        }
        return res;
    }
    ...
    public static String formatHMSM(long t) {
        ...
        if (hours > 0) {
            res = String.format(TEXT[26],
                    new Object[] {Integer.valueOf(hours), Integer.valueOf(minutes), Integer.valueOf(seconds) });
        } else {
            res = String.format(TEXT[27], new Object[] {Integer.valueOf(minutes), Integer.valueOf(seconds) });
        }
        return res;
    }
    ...
    public void updateFastestVictory(long newTime) {
        if (newTime < this.fastestTime || this.fastestTime == 0L) {
            ...
            this.pref.putLong("FAST_VICTORY", this.fastestTime);
            this.pref.flush();
    ...
    public void render(SpriteBatch sb, float screenX, float renderY) {
        FontHelper.renderSmartText(sb, FontHelper.panelNameFont, this.info, screenX + 75.0F * Settings.scale,
                renderY + 766.0F * Settings.yScale, 9999.0F, 38.0F * Settings.scale, Settings.CREAM_COLOR);

        if (this.info2 != null)
            FontHelper.renderSmartText(sb, FontHelper.panelNameFont, this.info2, screenX + 675.0F * Settings.scale,
                    renderY + 766.0F * Settings.yScale, 9999.0F, 38.0F * Settings.scale, Settings.CREAM_COLOR);
    }
}
 ...
 * CharStat.class Java compiler version: 8 (52.0) JD-Core Version: 1.1.3
 */
