...
public class RunPathElement {
    ...
    public enum PathNodeType {
        ERROR, MONSTER, ELITE, EVENT, BOSS, TREASURE, BOSS_TREASURE, CAMPFIRE, SHOP, UNKNOWN_MONSTER, UNKNOWN_SHOP, UNKNOWN_TREASURE, HEART;
    }
    ...
    public RunPathElement(String roomKey, int floorNum) {
        ...
        this.nodeType = pathNodeTypeForRoomKey(roomKey);
        this.floor = floorNum;
    }
    ...
    public void addRelicObtainStats(String relicKey) {
        addRelicObtainStats(Arrays.asList(new String[] {relicKey }));
    }
    ...
    public void update() {
        ...
        if (this.hb.hovered) {
            ...
            String header = String.format(TEXT_SIMPLE_FLOOR_FORMAT, new Object[] {Integer.valueOf(this.floor) });
            TipHelper.renderGenericTip(tipX, tipY, header, getTipDescriptionText());
        }
    }

    private String getTipHeaderWithRoomTypeText() {
        return String.format(TEXT_FLOOR_FORMAT, new Object[] {Integer.toString(this.floor), stringForType() });
    }
    ...
    private String getTipDescriptionText() {
        ...
        if (displayHP) {
            ...
            sb.append(String.format(TEXT_COMBAT_HP_FORMAT, new Object[] {this.currentHP, this.maxHP }));
            sb.append(" TAB ");
        }

        boolean displayGold = (this.gold != null);
        if (displayGold) {
            sb.append(String.format(TEXT_GOLD_FORMAT, new Object[] {this.gold }));
        }
        ...
        if (this.eventStats != null) {
            ...
            sb.append(localizedEventNameForKey(this.eventStats.event_name));

            if (!this.eventStats.player_choice.equals("Took Portal")) {

                if (this.eventStats.max_hp_gain == 0 && this.eventStats.max_hp_loss == 0
                        && this.eventStats.gold_loss == 0 && this.eventStats.gold_gain == 0
                        && this.eventStats.damage_taken == 0 && this.eventStats.damage_healed == 0
                        && this.eventStats.cards_removed == null && this.eventStats.cards_obtained == null
                        && this.eventStats.cards_transformed == null && this.eventStats.cards_upgraded == null
                        && this.eventStats.relics_obtained == null && this.eventStats.relics_lost == null
                        && this.eventStats.potions_obtained == null && this.battleStats == null) {
            ...
            if (this.eventStats.relics_lost != null && !this.eventStats.relics_lost.isEmpty()) {
                ...
                for (int i = 0; i < this.eventStats.relics_lost.size(); i++) {
                    ...
                    sb.append(" TAB ").append(String.format(TEXT_LOST_RELIC, new Object[] {relicName }));
            ...
            if (this.eventStats.max_hp_loss != 0) {
                ...
                sb.append(String.format(TEXT_GENERIC_MAX_HP_FORMAT,
                        new Object[] {Integer.valueOf(this.eventStats.max_hp_loss) }));
            }

            if (this.eventStats.damage_taken != 0) {
                sb.append(" NL ");
                sb.append(" TAB ").append(TEXT_TOOK);
                sb.append(String.format(TEXT_EVENT_DAMAGE,
                        new Object[] {Integer.valueOf(this.eventStats.damage_taken) }));
            }

            if (this.eventStats.gold_loss != 0) {
                sb.append(" NL ");
                sb.append(" TAB ").append(TEXT_SPENT);
                sb.append(String.format(TEXT_GOLD_FORMAT, new Object[] {Integer.valueOf(this.eventStats.gold_loss) }));
            }
            ...
            if (this.eventStats.max_hp_gain != 0) {
                ...
                sb.append(String.format(TEXT_GENERIC_MAX_HP_FORMAT,
                        new Object[] {Integer.valueOf(this.eventStats.max_hp_gain) }));
            }

            if (this.eventStats.damage_healed != 0) {
                sb.append(" NL ");
                sb.append(" TAB ").append(TEXT_HEALED);
                sb.append(String.format(TEXT_GENERIC_HP_FORMAT,
                        new Object[] {Integer.valueOf(this.eventStats.damage_healed) }));
            }

            if (this.eventStats.gold_gain != 0) {
                sb.append(" NL ");
                sb.append(" TAB ").append(TEXT_GAINED);
                sb.append(String.format(TEXT_GOLD_FORMAT, new Object[] {Integer.valueOf(this.eventStats.gold_gain) }));
            }

            if (this.eventStats.cards_removed != null && !this.eventStats.cards_removed.isEmpty()) {
                sb.append(" NL ");
                for (int i = 0; i < this.eventStats.cards_removed.size(); i++) {
                    String cardID = this.eventStats.cards_removed.get(i);
                    String cardName = CardLibrary.getCardNameFromMetricID(cardID);
                    sb.append(" TAB ").append(String.format(TEXT_REMOVE_OPTION, new Object[] {cardName }));
                    if (i < this.eventStats.cards_removed.size() - 1) {
                        sb.append(" NL ");
                    }
                }
            }
            ...
            if (this.eventStats.cards_upgraded != null && !this.eventStats.cards_upgraded.isEmpty()) {
                sb.append(" NL ");
                for (int i = 0; i < this.eventStats.cards_upgraded.size(); i++) {
                    String cardID = this.eventStats.cards_upgraded.get(i);
                    String cardName = CardLibrary.getCardNameFromMetricID(cardID);
                    sb.append(" TAB ").append(String.format(TEXT_UPGRADED, new Object[] {cardName }));
                    if (i < this.eventStats.cards_upgraded.size() - 1) {
                        sb.append(" NL ");
                    }
                }
            }
            ...
            if (this.eventStats.cards_transformed != null && !this.eventStats.cards_transformed.isEmpty()) {
                sb.append(" NL ");
                for (int i = 0; i < this.eventStats.cards_transformed.size(); i++) {
                    String cardID = this.eventStats.cards_transformed.get(i);
                    String cardName = CardLibrary.getCardNameFromMetricID(cardID);
                    sb.append(" TAB ").append(String.format(TEXT_TRANSFORMED, new Object[] {cardName }));
                    if (i < this.eventStats.cards_transformed.size() - 1) {
                        sb.append(" NL ");
                    }
                }
            }

            if (((this.eventStats.cards_obtained != null && !this.eventStats.cards_obtained.isEmpty())
                    || (this.eventStats.relics_obtained != null &&
                            !this.eventStats.relics_obtained.isEmpty())
                    || (this.eventStats.potions_obtained != null &&
                            !this.eventStats.potions_obtained.isEmpty()))
                    && this.relicsObtained == null && this.battleStats == null && this.cardChoiceStats == null
                ...
                if (this.eventStats.relics_obtained != null && !this.eventStats.relics_obtained.isEmpty()) {
                    ...
                    for (int i = 0; i < this.eventStats.relics_obtained.size(); i++) {
                        String name = (RelicLibrary.getRelic((String) this.eventStats.relics_obtained.get(i))).name;
                        if (i > 0) {
                            sb.append(" NL ");
                        }
                ...
                if (this.eventStats.cards_obtained != null && !this.eventStats.cards_obtained.isEmpty() &&
                        !this.eventStats.cards_obtained.isEmpty() && !this.eventStats.cards_obtained.equals("SKIP")) {
                    if (sb.length() > 0) {
                        sb.append(" NL ");
                    }
                    for (int i = 0; i < this.eventStats.cards_obtained.size(); i++) {
                        String name = CardLibrary.getCardNameFromMetricID(this.eventStats.cards_obtained.get(i));
                        if (i > 0) {
                            sb.append(" NL ");
                        }
        ...
        if (this.battleStats != null) {
            ...
            sb.append(localizedEnemyNameForKey(this.battleStats.enemies));
            sb.append(" NL ");
            sb.append(" TAB ").append(
                    String.format(TEXT_DAMAGE_FORMAT, new Object[] {Integer.valueOf(this.battleStats.damage) }));
            sb.append(" NL ");
            sb.append(" TAB ")
                    .append(String.format(TEXT_TURNS_FORMAT, new Object[] {Integer.valueOf(this.battleStats.turns) }));
        }
        ...
        if (this.campfireChoice != null) {
            ...
            switch (this.campfireChoice.key) {
                ...
                case "SMITH":
                    sb.append(
                            String.format(TEXT_SMITH_OPTION,
                                    new Object[] {CardLibrary.getCardNameFromMetricID(this.campfireChoice.data) }));
                    break;
                case "PURGE":
                    sb.append(
                            String.format(TEXT_TOKE_OPTION,
                                    new Object[] {CardLibrary.getCardNameFromMetricID(this.campfireChoice.data) }));
                    break;
                case "DIG":
                    ...
                    break;
                case "LIFT":
                    sb.append(String.format(TEXT_LIFT_OPTION,
                            new Object[] {this.campfireChoice.data, Integer.valueOf(3) }));
                    break;
                case "RECALL":
        ...
        if (showRelic) {
            ...
            for (int i = 0; i < this.relicsObtained.size(); i++) {
                String name = (RelicLibrary.getRelic((String) this.relicsObtained.get(i))).name;
                if (i > 0) {
                    sb.append(" NL ");
                }
                sb.append(" TAB ").append(TEXT_OBTAIN_TYPE_RELIC).append(name);
        ...
        if (showCards &&
                !this.cardChoiceStats.picked.isEmpty() && !this.cardChoiceStats.picked.equals("SKIP")) {
            String text;
        ...
        if (this.shopPurchases != null) {
            ...
            for (String key : this.shopPurchases) {
                ...
                if (CardLibrary.isACard(key)) {
                    ...
                    text = TEXT_OBTAIN_TYPE_RELIC + (RelicLibrary.getRelic(key)).name;
                } else if (PotionHelper.isAPotion(key)) {
                    text = TEXT_OBTAIN_TYPE_POTION + (PotionHelper.getPotion(key)).name;
                }
        ...
        if (this.shopPurges != null) {
            for (String key : this.shopPurges) {
                sb.append(" NL ").append(
                        String.format(TEXT_REMOVE_OPTION, new Object[] {CardLibrary.getCardNameFromMetricID(key) }));
            }
        }
    ...
    public void render(SpriteBatch sb) {
        Texture image = imageForType(this.nodeType);
        if (isHovered()) {
    ...
    private String stringForType() {
        return stringForType(this.nodeType);
    }
 ...
 * \RunPathElement.class Java compiler version: 8 (52.0) JD-Core Version: 1.1.3
 */
