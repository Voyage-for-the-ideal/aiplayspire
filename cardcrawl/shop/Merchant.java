...
public class Merchant implements Disposable {
    ...
    public Merchant(float x, float y, int newShopScreen) {
        ...
        this.shopScreen = 1;
        this.anim = new AnimatedNpc(DRAW_X + (256.0f * Settings.scale),
                AbstractDungeon.floorY + (30.0f * Settings.scale), "images/npcs/merchant/skeleton.atlas",
        ...
        while (true) {
            ...
            makeCopy = AbstractDungeon.getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.ATTACK, true)
                    .makeCopy();
        }
        this.cards1.add(c);
        AbstractCard makeCopy2 = AbstractDungeon
                .getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.ATTACK, true).makeCopy();
        while (true) {
            c2 = makeCopy2;
            if (!Objects.equals(c2.cardID, this.cards1.get(this.cards1.size() - 1).cardID)
                    && c2.color != AbstractCard.CardColor.COLORLESS) {
                break;
            }
            makeCopy2 = AbstractDungeon
                    .getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.ATTACK, true).makeCopy();
        }
        this.cards1.add(c2);
        AbstractCard makeCopy3 = AbstractDungeon
                .getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.SKILL, true).makeCopy();
        while (true) {
            c3 = makeCopy3;
            if (c3.color != AbstractCard.CardColor.COLORLESS) {
                break;
            }
            makeCopy3 = AbstractDungeon.getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.SKILL, true)
                    .makeCopy();
        }
        this.cards1.add(c3);
        AbstractCard makeCopy4 = AbstractDungeon
                .getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.SKILL, true).makeCopy();
        while (true) {
            c4 = makeCopy4;
            if (!Objects.equals(c4.cardID, this.cards1.get(this.cards1.size() - 1).cardID)
                    && c4.color != AbstractCard.CardColor.COLORLESS) {
                break;
            }
            makeCopy4 = AbstractDungeon.getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.SKILL, true)
                    .makeCopy();
        }
        this.cards1.add(c4);
        AbstractCard makeCopy5 = AbstractDungeon
                .getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.POWER, true).makeCopy();
        while (true) {
            c5 = makeCopy5;
            if (c5.color != AbstractCard.CardColor.COLORLESS) {
                break;
            }
            makeCopy5 = AbstractDungeon.getCardFromPool(AbstractDungeon.rollRarity(), AbstractCard.CardType.POWER, true)
                    .makeCopy();
        }
        this.cards1.add(c5);
    ...
    public void update() {
        ...
        if (((this.hb.hovered && InputHelper.justClickedLeft) || CInputActionSet.select.isJustPressed())
                && !AbstractDungeon.isScreenUp && !AbstractDungeon.isFadingOut
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(ImageMaster.MERCHANT_RUG_IMG, DRAW_X + this.modX, DRAW_Y + this.modY, 512.0f * Settings.scale,
                512.0f * Settings.scale);
        if (this.hb.hovered) {
        ...
        if (Settings.isControllerMode) {
            ...
            sb.draw(CInputActionSet.select.getKeyImg(), (DRAW_X - 32.0f) + (150.0f * Settings.scale),
                    (DRAW_Y - 32.0f) + (100.0f * Settings.scale), 32.0f, 32.0f, 64.0f, 64.0f, Settings.scale,
