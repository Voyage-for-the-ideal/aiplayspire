...
public class MetricData {
    ...
    public void addShopPurchaseData(String key) {
        if (this.items_purchased.size() == this.item_purchase_floors.size()) {
            this.item_purchase_floors.add(Integer.valueOf(AbstractDungeon.floorNum));
        }
    ...
    public void addPurgedItem(String key) {
        if (this.items_purged.size() == this.items_purged_floors.size()) {
            this.items_purged_floors.add(Integer.valueOf(AbstractDungeon.floorNum));
        }
...
/*
 * Location:
 * E:\代码\SlayTheSpire\desktop-1.0.jar!\com\megacrit\cardcrawl\metrics\MetricData
